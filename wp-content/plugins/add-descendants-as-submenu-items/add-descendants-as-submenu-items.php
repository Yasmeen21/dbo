...
class Add_Descendants_As_Submenu_Items {
			...
			foreach ( $children as $child ) {
				...
				//if ( $child->type=='taxonomy' ) {
					...
					foreach ( $post_children as $post_child ) {
						$post_child = wp_setup_nav_menu_item( $post_child );
						$post_child->db_id = $post_child->ID;

						$this->added[$post_child->ID] = true;
						$post_child->menu_item_parent = $child->ID;
						$menu_order++;
						$post_child->menu_order = $menu_order;
	...
	public function fix_menu_current_item( $items ) {
		...
		if ( isset( $queried_object->term_id ) ) {
			...
			//$parent_field = 'post_parent';
			//$type = 'post_type';
			$ancestors = wp_get_post_categories( $queried_object_id );
			$parent_field = 'parent';
			$type = 'taxonomy';

			foreach ( $ancestors as $ancestor ) {
				$more_ancestors = get_ancestors( $ancestor, 'category' );
				$ancestors[] = $more_ancestors;
			}
		} else {
			return $items;
		}
