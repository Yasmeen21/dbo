...
class adsns
{
	...
	function adsns_end_post_ad( $content ) {
		...
		if ( ! is_feed() && is_single() && $adsns_count < $this->adsns_options[ 'max_ads' ] && $adsns_count < $this->adsns_options['max_homepostads'] ) {  // Checking if we are on a single page
			...
			//DF: plugin is missing some variables, so just hardcode from adsense generated codes
			$content.= '<div id="end_post_ad" class="ads">
				<script type="text/javascript"><!--
				google_ad_client = "ca-pub-6511226955130242";
				/* DBO */
				google_ad_slot = "9375746518";
				google_ad_width = 234;
				google_ad_height = 60;
				//-->
				</script>
